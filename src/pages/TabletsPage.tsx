...
export const TabletsPage = () => {
  const [products, setProducts] = useState<Product<Tablet>[]>([]);
  ...
  useEffect(() => {
    ...
    setErrorMessage("");
    setLoading(true);
    getProductsByCategory<Tablet>("tablets", +params.page, perPageValue)
      .then((response) => {
  ...
  }, [searchParams, params, perPageValue]);
  ...
  return (
    <div>
      <h1 className="text-4xl font-mont-bold text-primary my-10">Tablets</h1>

      {loading && <ClipLoader color="#313237" loading={true} size={50} />}

      {errorMessage && <p className="text-2xl text-red-500">{errorMessage}</p>}

      {total === 0 && !loading && <p>There are no tablets on the server</p>}
